...
def insert_polution(img, center, circle_radius):
    for n in range(circle_radius):
        cv.circle(img, center, 5 * (n + 1), (n * 2, n * 8, 255 - 2 * n))
    return True


src2 = src2[240:440, 100:400][:]
bufd = src2.copy()
bufc = src2.copy()
#map_r = cv.imread('img_r_test.png', cv.IMREAD_GRAYSCALE)
map_r = map_r[240:440, 100:400]
for i in range(np.shape(map_r)[0]):
    for j in range(np.shape(map_r)[1]):
        if map_r[i][j] == 255:
            map_r[i][j] = 0
ind = np.where(map_r)
...
binar = binar[240:440, 100:400]
cv.addWeighted(bufc, alpha, src2, 1 - alpha, 2.0, src2)
src2[binar == 237] = 255

cv.imwrite("real.png", src2)
print()
